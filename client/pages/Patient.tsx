    ...
    <div className="container py-10">
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
        ...
        <div>
          ...
          {tab === "doctors" && <Doctors />}
          {tab === "appointments" && (
            <MyAppointments userEmail={user?.email || ""} />
          )}
          {tab === "prescriptions" && (
            <MyPrescriptions userEmail={user?.email || ""} />
          )}
      ...
      try {
        const res = await fetch(
          "/api/appointments?email=" +
            encodeURIComponent(localStorage.getItem("user_email") || ""),
        );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const d = await res.json();
        const prescs = (d.appointments || []).flatMap(
          (a: any) => a.prescriptions || [],
        );
  ...
  const filtered = useMemo(() => {
    ...
    return providers.filter((p) => {
      ...
      if (
        ...
        return false;
      if (q && !p.displayName.toLowerCase().includes(q.toLowerCase()))
        return false;
      return true;
    });
          ...
          <Card key={p.userId}>
            <CardContent className="py-3 flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {p.specialty} • {p.languages?.join(", ")}
                </div>
              </div>
              <div className="flex gap-2">
                <Button onClick={() => bookQuick(p.providerId || p.userId)}>
                  Book
                </Button>
              </div>
            </CardContent>
...
async function bookQuick(providerId: string) {
  ...
  const body = {
    patientName: localStorage.getItem("user_name") || "Patient",
...
function MyAppointments({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(
          "/api/appointments?email=" + encodeURIComponent(userEmail),
        );
        const d = await res.json();
        setItems(d.appointments || []);
      } catch (err) {
            ...
            <Card key={a.id}>
              <CardContent className="py-3 flex items-center justify-between">
                <div>
                  ...
                  {(a as any).paymentStatus === "success" ? (
                    <div className="text-xs text-emerald-600">
                      Paid
                      {(a as any).paidAt
                        ? ` • ${new Date((a as any).paidAt).toLocaleString()}`
                        : ""}
                    </div>
                  ) : (
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                  ...
                  <Button variant="outline" onClick={() => reschedule(a.id)}>
                    Reschedule
                  </Button>
                  {(a as any).paymentStatus !== "success" && (
                    <Button onClick={() => payForAppointment(a.id)}>Pay</Button>
                  )}
                </div>
              </CardContent>
            </Card>
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const d = await res.json();
        const files = (d.appointments || [])
          .flatMap((a: any) => a.files || [])
          .slice(0, 50);
        setDocs(files || []);
      } catch (err) {
        setDocs([]);
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              reader.onload = async () => {
                ...
                try {
                  await fetch("/api/appointments/unknown/files", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      filename: file.name,
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const d = await res.json();
        const inv = (d.appointments || []).map((a: any) => ({
          id: a.id,
          date: a.scheduledAt,
          amount: a.feeUSD || 120,
      ...
      <div className="grid gap-2">
        {invoices.map((i) => (
          <Card key={i.id}>
            <CardContent className="py-3 flex items-center justify-between">
              <div>
                <div className="font-medium">Invoice {i.id}</div>
                <div className="text-sm text-muted-foreground">
                  {new Date(i.date).toLocaleDateString()} • ${i.amount}
                </div>
              </div>
              <div className="text-sm capitalize">{i.status}</div>
            </CardContent>
          </Card>
        ))}
      </div>
...
function Support({ userEmail }: { userEmail: string }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      console.warn("Ticket failed", err);
  ...
  return (
    <div>
      ...
      <div>
        ...
        {tickets.map((t) => (
          <Card key={t.id}>
            <CardContent className="py-2 text-sm">{t.message}</CardContent>
          </Card>
        ))}
      </div>
    </div>
...
function Settings() {
  ...
  const send2fa = async () => {
    ...
    alert(
      "A verification code was sent to your phone (simulated). Enter code to verify.",
    );
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setProfile({ ...profile, insurance: e.target.value })
                  }
                />
              </div>
              <div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button onClick={send2fa}>
                  {enable2fa ? "Re-verify" : "Enable 2FA"}
                </Button>
              </div>
              <div className="text-xs text-muted-foreground">
                For production, integrate Twilio Verify or Supabase OTP for
                SMS/email 2FA.
              </div>
            </div>
          </CardContent>
