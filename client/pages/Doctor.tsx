  ...
  const refreshAppts = async () => {
    ...
    if (supabase) {
      try {
        ...
        console.warn(
          "Supabase fetch appointments error, falling back to API:",
          error,
        );
      } catch (err) {
        console.warn(
          "Supabase fetch appointments exception, falling back to API:",
          err,
        );
      }
    }
  ...
  const loadProfile = async () => {
    ...
    if (supabase) {
      try {
        ...
        console.warn(
          "Supabase fetch profile error, falling back to API:",
          error,
        );
      } catch (err) {
        console.warn(
          "Supabase fetch profile exception, falling back to API:",
          err,
        );
      }
    }
    ...
    <div className="container py-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {notifications &&
            (notifications.newAppointments > 0 ||
              notifications.messages > 0) && (
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10">
                {notifications.newAppointments > 0 && (
                  <div className="text-sm">
                    📅 {notifications.newAppointments} new
                  </div>
                )}
                {notifications.messages > 0 && (
                  <div className="text-sm">💬 {notifications.messages}</div>
                )}
              </div>
            )}
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
        ...
        <div>
                    ...
                    <CardContent className="py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="outline"
                          onClick={() =>
                            setStatus(a.id, "completed", refreshAppts)
                          }
                        >
                          Mark completed
                        </Button>
                        <Button
                          variant="destructive"
                          onClick={() =>
                            setStatus(a.id, "cancelled", refreshAppts)
                          }
                        >
                          Cancel
                        </Button>
          ...
          {/* Consultations */}
          {tab === "consult" && (
            <Consultations items={items} refresh={refreshAppts} />
          )}
          ...
          {tab === "prescriptions" && (
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Use the Consultations tab to issue e-prescriptions. Backend
                hooks exist at /api/appointments/:id/prescriptions
              </p>
            </div>
          )}
          ...
          {/* Earnings */}
          {tab === "earnings" && (
            <Earnings providerId={user?.providerId || ""} />
          )}

          {/* Settings */}
          {tab === "settings" && (
            <Settings profile={profile} onChange={setProfile} />
          )}
        </div>
      </div>
    </div>
...
async function setStatus(
  ...
  if (supabase) {
    try {
      ...
      console.warn(
        "Supabase update status exception, falling back to API:",
        err,
      );
    }
  }
...
function Consultations({
    ...
    (async () => {
      ...
      if (supabase) {
        try {
          ...
          console.warn(
            "Supabase get messages error, falling back to API:",
            error,
          );
        } catch (err) {
          console.warn(
            "Supabase get messages exception, falling back to API:",
            err,
          );
        }
      }
  ...
  const sendNote = async () => {
    ...
    await fetch(`/api/appointments/${selected}/notes`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ body: note, authorUserId: user?.id }),
    });
    setNote("");
    refresh();
  };
  ...
  const createMeeting = async () => {
    if (!selected) return;
    try {
      const res = await fetch(`/api/appointments/${selected}/create-meeting`, {
        method: "POST",
      });
      if (!res.ok) return;
      await res.json();
    ...
    if (supabase) {
      try {
        ...
        if (!fetchErr && appt) {
          ...
          console.warn(
            "Supabase add prescription error, falling back to API:",
            error,
          );
        }
      } catch (err) {
        console.warn(
          "Supabase add prescription exception, falling back to API:",
          err,
        );
      }
    }
    await fetch(`/api/appointments/${selected}/prescriptions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...rx,
    ...
    if (supabase) {
      try {
        ...
        if (!fetchErr && appt) {
          ...
          console.warn(
            "Supabase post message error, falling back to API:",
            error,
          );
        }
      } catch (err) {
        console.warn(
          "Supabase post message exception, falling back to API:",
          err,
        );
      }
    }
    await fetch(`/api/appointments/${selected}/messages`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: chatInput, authorUserId: user?.id }),
    });
    setChatInput("");
    const res = await fetch(`/api/appointments/${selected}/messages`);
    const data = await res.json();
              ...
              <div className="space-y-3">
                ...
                {appt.intake && (
                  <div className="text-sm text-muted-foreground">
                    ...
                    {appt.intake.medications && (
                      <div>Medications: {appt.intake.medications}</div>
                    )}
                    {appt.intake.allergies && (
                      <div>Allergies: {appt.intake.allergies}</div>
                    )}
                  </div>
                )}
                <div>
                ...
                <div>
                  ...
                  <div className="grid sm:grid-cols-3 gap-2">
                    <Input
                      ...
                      value={rx.medication}
                      onChange={(e) =>
                        setRx({ ...rx, medication: e.target.value })
                      }
                    />
                    <Input
                      placeholder="Dosage"
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setRx({ ...rx, instructions: e.target.value })
                      }
                    />
                  </div>
                  <div className="mt-2">
                    <label className="text-sm">
                      E-sign (type full name to sign)
                    </label>
                    <Input
                  ...
                  <div className="mt-2 flex justify-end">
                    <Button onClick={sendRx}>Issue prescription</Button>
                  </div>
                </div>
                        ...
                        reader.onload = async () => {
                          ...
                          if (supabase) {
                            try {
                              const bucket =
                                (import.meta.env
                                  .VITE_SUPABASE_LICENSE_BUCKET as string) ||
                                "attachments";
                          ...
                          try {
                            const res = await fetch(
                              `/api/appointments/${selected}/files`,
                              {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
...
function Patients({ items }: { items: Appointment[] }) {
  ...
  const list = useMemo(() => {
    const map = new Map<
      string,
      { name: string; email: string; last: string }
    >();
    for (const a of items) {
      const m = map.get(a.patientEmail) || {
        name: a.patientName,
                ...
                history.map((h) => (
                  <Card key={h.id}>
                    <CardContent className="py-3 text-sm">
                      ...
                      <div className="mt-2">Reason: {h.reason}</div>
                    </CardContent>
                  </Card>
                ))}
            </div>
    ...
    if (supabase) {
      try {
        const { data: appt, error: fetchErr } = await supabase
          ...
          .single();
        if (!fetchErr && appt) {
          const next = (appt.reminders || []).concat(
          ...
          console.warn(
            "Supabase add reminder error, falling back to API:",
            error,
          );
        }
      } catch (err) {
        console.warn(
          "Supabase add reminder exception, falling back to API:",
          err,
        );
      }
    }
    await fetch(`/api/appointments/${id}/reminders`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function Earnings({ providerId }: { providerId: string }) {
      ...
      if (supabase) {
        try {
          ...
          if (!error && Array.isArray(appts)) {
            const completed = (appts as any[]).filter(
            ...
            const byWeek = Array.from(byWeekMap.entries()).map(([week, v]) => ({
              week,
              revenue: v.revenue,
              sessions: v.sessions,
            }));
          ...
          console.warn(
            "Supabase earnings exception, falling back to API:",
            err,
          );
        }
      }
  ...
  const exportCsv = () => {
    ...
    for (const r of data?.byWeek || [])
      rows.push([`"${r.week}"`, r.revenue, r.sessions].join(","));
    rows.push("");
    ...
    a.href = url;
    a.download = `earnings-${providerId || "me"}.csv`;
  ...
  const createPayout = async () => {
    ...
    try {
      ...
      console.warn("Create payout failed", err);
    }
    setCreating(false);
  };
        ...
        <CardContent>
          <div className="mb-4">
            ...
            <div className="text-xs text-muted-foreground mt-2">
              Payouts are simulated in-memory. Connect Stripe to process real
              payouts.
            </div>
          </div>
...
function Settings({
  ...
  const save = async () => {
    if (supabase) {
      try {
        ...
        const { data, error } = await supabase
          .from("doctors")
          .upsert({ ...payload, userId: form.userId })
          .select()
          .single();
        if (!error) {
          onChange(data || form);
        ...
        console.warn(
          "Supabase upsert profile error, falling back to API:",
          error,
        );
      } catch (err) {
        console.warn(
          "Supabase upsert profile exception, falling back to API:",
          err,
        );
      }
    }
  ...
  const setAvail = (i: number, key: "day" | "start" | "end", val: any) => {
    ...
    next[i] = {
      ...next[i],
      [key]: key === "day" ? Number(val) : String(val),
    } as any;
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div>
                  ...
                  reader.onload = async () => {
                    ...
                    if (supabase) {
                      try {
                        ...
                        console.warn(
                          "Supabase client upload error, falling back to server API:",
                          error,
                        );
                      } catch (err) {
                        console.warn(
                          "Supabase client upload exception, falling back to server API:",
                          err,
                        );
                      }
                    }
                    ...
                    try {
                      const res = await fetch("/api/doctor/me/license", {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                          ...(token
            ...
            <div className="text-xs text-muted-foreground mt-2">
              Upload a clear photo or PDF of your medical license. The file will
              be stored securely; admin will verify it.
            </div>
          </div>
          <div>
            <label className="text-sm font-medium">
          ...
          <div>
            ...
            <Input
              ...
              value={form.feeUSD}
              onChange={(e) =>
                setForm({ ...form, feeUSD: Number(e.target.value) })
              }
            />
          </div>
          ...
          {form.availability.map((s, i) => (
            <div key={i} className="grid grid-cols-12 gap-2 items-center">
              <Input
                ...
                max={6}
                value={s.day}
                onChange={(e) => setAvail(i, "day", e.target.value)}
              />
              <Input
                className="col-span-4"
                type="time"
                value={s.start}
                onChange={(e) => setAvail(i, "start", e.target.value)}
              />
              <Input
                className="col-span-4"
                type="time"
                value={s.end}
                onChange={(e) => setAvail(i, "end", e.target.value)}
              />
