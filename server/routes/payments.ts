...
async function upsertPayment(tx: PaymentTransaction) {
  ...
  if (!supabase) {
    memoryPayments[tx.reference] = tx;
    return { ok: true };
  }
  const { error } = await supabase
    .from("payments")
    .upsert(tx, { onConflict: "reference" });
  if (error) {
    console.warn("[Paystack] Supabase upsert payment error", error);
    memoryPayments[tx.reference] = tx;
  }
  return { ok: !error };
}
...
async function updateAppointmentPaid(
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          paymentStatus: "paid",
          paidAt: nowISO(),
          feeUSD: fromMinorUnits(amountMinor),
        })
        .eq("id", appointmentId);
      if (error) console.warn("[Paystack] Supabase mark paid error", error);
    } catch (err) {
      console.warn("[Paystack] Supabase mark paid exception", err);
    }
  }
  const appt = apptStore.find((a) => a.id === appointmentId);
  if (appt) {
    (appt as any).paymentStatus = "paid";
    (appt as any).paidAt = nowISO();
    if ((appt as any).feeUSD == null)
      (appt as any).feeUSD = fromMinorUnits(amountMinor);
  }
}
...
export const initiatePayment: RequestHandler = async (req, res) => {
  try {
    ...
    const { appointmentId, amount, currency } = (req.body || {}) as {
      appointmentId?: string;
      amount?: number;
      currency?: string;
    };
    ...
    const callback = callbackBase
      ? `${callbackBase.replace(/\/$/, "")}/payment-status?reference=${encodeURIComponent(reference)}`
      : undefined;
    ...
    const body = {
      ...
      currency: curr,
      reference,
      metadata: { appointmentId, email },
      ...(callback ? { callback_url: callback } : {}),
    } as any;

    const rs = await fetch(paystackInitUrl, {
      method: "POST",
      headers: {
...
export const verifyPayment: RequestHandler = async (req, res) => {
  try {
    ...
    const rs = await fetch(verifyUrl, {
    ...
    if (!json.status)
      return res
        .status(400)
        .json({ ok: false, message: json.message || "Verification failed" });

    const data = json.data || {};
    const status = data.status as string;
    const appointmentId =
      (data.metadata && data.metadata.appointmentId) || undefined;
    ...
    const existing =
      ...
      ({
        ...
        patientEmail: data.customer?.email || "",
...
export const paystackWebhook: RequestHandler = async (req, res) => {
  try {
    ...
    if (event.event === "charge.success" && event.data?.status === "success") {
      ...
      const appointmentId = event.data.metadata?.appointmentId as
        | string
        | undefined;
      const amountMinor = event.data.amount as number;

      const existing =
        (await getPayment(ref)) ||
        ({
          id: crypto.randomUUID(),
          provider: "paystack",
          reference: ref,
          appointmentId: appointmentId || "",
          patientEmail: event.data.customer?.email || "",
          amount: (amountMinor ?? 0) / 100,
