...
export const initiateFlutterwave: RequestHandler = async (req, res) => {
  const { appointmentId, email, name, amount, callbackUrl } = (req.body ||
    {}) as {
  ...
  if (!secret)
    return res
      .status(500)
      .json({ error: "Flutterwave keys not configured" } as ApiError);
  ...
  try {
    ...
    if (!r.ok || !data?.status || data.status !== "success") {
      ...
      return res
        .status(400)
        .json({ error: `Flutterwave init failed: ${msg}` } as ApiError);
    }
    ...
    res
      .status(500)
      .json({
        error: `Flutterwave init exception: ${err?.message || err}`,
      } as ApiError);
  }
};
...
export const verifyFlutterwave: RequestHandler = async (req, res) => {
  ...
  if (!tx_id && !tx_ref)
    return res
      .status(400)
      .json({ error: "Missing transaction_id or tx_ref" } as ApiError);
  try {
    ...
    if (tx_id) {
      ...
      if (!r.ok || data.status !== "success") {
        ...
        return res
          .status(400)
          .json({ error: `Flutterwave verify failed: ${msg}` } as ApiError);
      }
      ...
      if (!receipt) {
        receipt = {
          id: genId(),
          provider: "flutterwave",
          appointmentId: String(data.data.meta?.appointmentId || "unknown"),
          reference: ref,
      ...
      receipt.status =
        data.data.status === "successful" ? "paid" : (data.data.status as any);
      if (receipt.status === "paid") {
        ...
        if (appt) {
          (appt as any).payments = ((appt as any).payments || []).concat(
            receipt,
          );
    ...
    res
      .status(500)
      .json({
        error: `Flutterwave verify exception: ${err?.message || err}`,
      } as ApiError);
  }
};

export const flutterwaveWebhook: RequestHandler = async (req, res) => {
  const signature = (req.headers["verif-hash"] || req.headers["verif_hash"]) as
    | string
    | undefined;
  ...
  if (
    ...
    if (!receipt) {
      receipt = {
        id: genId(),
        provider: "flutterwave",
        appointmentId: String(event.data.meta?.appointmentId || "unknown"),
        reference: ref,
        amount: Number(event.data.amount),
        currency: event.data.currency || currency(),
        status: "paid",
        customerEmail: String(event.data.customer?.email || ""),
        createdAt: new Date().toISOString(),
      };
    }
    receipt.status = "paid";
    receipt.paidAt = new Date(
      event.data.created_at || Date.now(),
    ).toISOString();
...
export const initiatePayout: RequestHandler = async (req, res) => {
  ...
  if (!amount || !account_bank || !account_number)
    return res.status(400).json({ error: "Missing payout fields" } as ApiError);
  const secret = process.env.FLUTTERWAVE_SECRET_KEY;
  if (!secret)
    return res
      .status(500)
  ...
  try {
    const r = await fetch("https://api.flutterwave.com/v3/transfers", {
      ...
      body: JSON.stringify({
        ...
        account_bank,
        account_number,
        narration: narration || "Doctor payout",
    ...
    if (!r.ok || data?.status !== "success") {
      ...
      return res
        .status(400)
        .json({ error: `Flutterwave payout failed: ${msg}` } as ApiError);
    }
    ...
    res
      .status(500)
      .json({
        error: `Flutterwave payout exception: ${err?.message || err}`,
      } as ApiError);
  }
};
